...
namespace MoreMountains.CorgiEngine
{
	...
	public class MMF_CorgiEngineSound : MMF_Feedback
	{
		[MMFInspectorGroup("Corgi Engine Sound", true, 17)]
		...
		protected override void CustomPlayFeedback(Vector3 position, float attenuation = 1.0f)
		{
			if (Active)
			{
				if (SoundFX != null)
				{
					_audioSource = MMSoundManagerSoundPlayEvent.Trigger(SoundFX, MMSoundManager.MMSoundManagerTracks.Sfx, Owner.transform.position, loop:Loop);
				}
			}
		}
