...
{
	/// <summary>
	/// A class used to store sound settings
	/// </summary>
	[Serializable]
	public class SoundSettings
	{
		/// whether or not music should be on
		[Tooltip("whether or not music should be on")]
		public bool MusicOn = true;
		/// whether or not sfx should be on
		[Tooltip("whether or not sfx should be on")]
	...
	{	
		...
		[Tooltip("the sound settings to use for this manager")]
		...
		[Tooltip("whether or not to mute sfx when pausing the game")]
			...
			AudioSource reuseSource = null, AudioMixerGroup audioGroup = null)
		{
			...
			if (audioSource == null)
			{
				...
				// we add an audio source to that host
				AudioSource newAudioSource = temporaryAudioHost.AddComponent<AudioSource>() as AudioSource;
